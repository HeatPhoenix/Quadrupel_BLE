...
public class DeviceBluetoothManager {
    ...
    private UUID uuid_device_service = UUID.fromString("6e400001-b5a3-f393-e0a9-e50e24dcca9e");

    // UUID of the RX characteristic of the device service
    private UUID uuid_device_rx_characteristic = UUID.fromString("6e400002-b5a3-f393-e0a9-e50e24dcca9e");

    // UUID of the TX characteristic of the device service
    private UUID uuid_device_tx_characteristic = UUID.fromString("6e400003-b5a3-f393-e0a9-e50e24dcca9e");

    // UUID of the descriptor for the characteristic
    private UUID uuid_device_characteristic_descriptor = UUID.fromString("00002902-0000-1000-8000-00805f9b34fb");
    ...
    */
    ...
    private class WriteDispatchTask extends AsyncTask<byte[], byte[], byte[]> {
        ...
        protected void onPostExecute(byte[] data) {
            ...
            DeviceBluetoothManager.this.tx_characteristic.setValue(data);

            System.out.println("Task: Got lock and issued write!");

            // Write characteristic
            boolean outcome = gatt.writeCharacteristic(tx_characteristic);
    ...
    */
    ...
    private BluetoothGattCallback gattCallback = new BluetoothGattCallback() {
        ...
        public void onServicesDiscovered (BluetoothGatt gatt, int status) {
            ...
            Log.i("onServicesDiscovered", "gatt status =" + status);
            ...
            if (didConnect == false) {
                ...
                tx_characteristic = service.getCharacteristic(uuid_device_rx_characteristic);

                rx_characteristic = service.getCharacteristic(uuid_device_tx_characteristic);

                // Set whether characteristic is enabled
                if (gatt.setCharacteristicNotification(tx_characteristic, true) == false) {
                    Log.e("BLE", "Characteristic could not be enabled!?");
                    return;
                }

                if (gatt.setCharacteristicNotification(rx_characteristic, true) == false) {
                    Log.e("BLE", "Characteristic could not be enabled!?");
                    return;
                }

                // Set the write-type for the characteristic
                tx_characteristic.setWriteType(BluetoothGattCharacteristic.WRITE_TYPE_DEFAULT);
                rx_characteristic.setWriteType(BluetoothGattCharacteristic.PROPERTY_NOTIFY);

                // For all descriptors, subscribe
                if (rx_characteristic.getDescriptors().size() > 0) {
                    for (BluetoothGattDescriptor d : rx_characteristic.getDescriptors()) {
                        Log.i("RX Descriptor", "BluetoothGattDescriptor: " + d.getUuid().toString());
                    }
                } else {
                    Log.e("RX Descriptor", "No BluetoothGattDescriptors");
                    //return;
                }

                // Now setup the descriptor to enable notifications (aperiodic feedback from device)
                BluetoothGattDescriptor descriptor = rx_characteristic.getDescriptor(uuid_device_characteristic_descriptor);
    ...
    */
    ...
    public DeviceBluetoothManager (Context context, DeviceBluetoothInterface delegate) {
        ...
        System.out.println("uuid_device_characteristic = " + uuid_device_rx_characteristic.toString());
